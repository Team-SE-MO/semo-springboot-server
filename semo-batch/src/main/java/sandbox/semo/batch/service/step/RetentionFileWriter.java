...
public class RetentionFileWriter implements ItemWriter<SessionData> {
    ...
    public void init() {
        try {
            ...
            fileWriter = new BufferedWriter(new FileWriter(backupPath));
            writeHeader();
    ...
    public void write(Chunk<? extends SessionData> chunk) throws Exception {
        try {
            for (SessionData sessionData : chunk) {
                String csvLine = formatCsvLine(sessionData);
                fileWriter.write(csvLine);
            }
            fileWriter.flush();  // chunk 단위로 한번만 flush
        } catch (IOException e) {
