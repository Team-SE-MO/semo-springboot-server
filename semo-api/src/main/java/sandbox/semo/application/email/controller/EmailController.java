...
import sandbox.semo.domain.member.dto.request.EmailVerify;
...
public class EmailController {
    ...
    @PostMapping
    public ApiResponse<String> sendEmail(@Valid @RequestBody EmailSend request) {
        emailService.processEmailRequest(request);
        return ApiResponse.successResponse(HttpStatus.OK, "이메일 전송 성공");
    }

    @PostMapping("/auth")
    public ApiResponse<String> verifyAuthCode(@RequestBody EmailVerify emailRequest) {
